...
<div class="bg-white rounded-lg shadow-sm">
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-gray-900">Detail Event</h1>
            <div class="flex space-x-2">
                <a href="{{ route('admin.events.edit', $event) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    Edit
                </a>
            ...
            <div class="lg:col-span-2">
                <div class="mb-6">
                    ...
                    <div class="flex items-center text-sm text-gray-500 space-x-4 mb-4">
                        <span>Status:
                            <span class="px-2 py-1 text-xs rounded-full {{ $event->is_published ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ $event->is_published ? 'Published' : 'Draft' }}
                            </span>
                        </span>
                        <span>Tanggal: {{ $event->event_date ? $event->event_date->format('d M Y') : 'Belum ditentukan' }}</span>
                    </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <p class="text-sm text-gray-900">{{ $event->slug }}</p>
                        </div>
                        @if($event->location)
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-900">{{ $event->start_time }}</p>
                        </div>
                        @endif
                        @if($event->end_time)
                        <div>
                            <span class="text-sm font-medium text-gray-500">Waktu Selesai:</span>
                            <p class="text-sm text-gray-900">{{ $event->end_time }}</p>
                        </div>
                        @endif
                        <div>
                            <span class="text-sm font-medium text-gray-500">Dibuat:</span>
                            <p class="text-sm text-gray-900">{{ $event->created_at->format('d M Y H:i') }}</p>
                        </div>
                        <div>
                            <span class="text-sm font-medium text-gray-500">Diupdate:</span>
                            <p class="text-sm text-gray-900">{{ $event->updated_at->format('d M Y H:i') }}</p>
                        </div>
                    </div>
                </div>
            </div>
