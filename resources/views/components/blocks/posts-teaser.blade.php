...
<section @if($anchor) id="{{ $anchor }}" @endif class="posts-teaser-block py-16 {{ $data['background_color'] ?? 'bg-gray-50' }}">
  <div class="container mx-auto px-4">
    ...
    @php
      ...
      $samplePosts = collect($data['_sample_posts'] ?? []);
      $usingSample = $posts->count() === 0 && $samplePosts->count() > 0;
    @endphp

    @if($posts->count() > 0 || $usingSample)
      <div class="grid md:grid-cols-{{ min($posts->count(), 3) }} gap-8">
        @foreach(($posts->count() ? $posts : $samplePosts) as $post)
          <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            @if(is_object($post) && $post->cover_path)
            ...
            <div class="p-6">
              ...
              <h3 class="text-xl font-semibold mb-3">
                ...
                @else
                  <span>{{ $post['title'] ?? 'Postingan' }}</span>
                @endif
              </h3>
              @php $excerpt = is_object($post) ? $post->excerpt : ($post['excerpt'] ?? null); @endphp
              @if($excerpt)
